...
def build_counts_dict(sequencing_filename):
    with open(sequencing_filename) as file:
        for i, line in enumerate(file):
            ...
            if strand == "+":
                if left not in counts_dict[chromosome]["+"]:
                    counts_dict[chromosome]["+"][left] = []

                counts_dict[chromosome][strand][left].append(right)
            else:
                if right not in counts_dict[chromosome]["-"]:
                    counts_dict[chromosome]["-"][right] = []

                counts_dict[chromosome][strand][right].append(left)
...
if __name__ == '__main__':
    ...
    for i, sequencing_filename in enumerate(sequencing_files_list):
        ...
        # Add it to the final_output
        final_output.append(pausing_pileups)

        header.append(sequencing_filename.split("/")[-1])

    with open(regions_filename.replace(".bed", "maxTSS-maxTPS.txt"), 'w') as output_file:
        output_writer = csv.writer(output_file, delimiter='\t', lineterminator='\n')
        output_writer.writerow(header)

        for i in range(len(final_output[0])):
            count_data = [x[i] for x in final_output]
            output_writer.writerow([i] + count_data)

...
def get_primes_data(regions_filename, three_prime_counts_dict):
...
def run_three_prime_metaplots(regions_filename, sequencing_files_list):
    ...
    for sequencing_file in sequencing_files_list:
        ...
        five_same_regions_data, five_rev_regions_data = get_primes_data(regions_filename, three_prime_counts_dict)

        # 3. Get averages data
        three_same_averages = get_averages(five_same_regions_data)
        three_rev_averages = get_averages(five_rev_regions_data)

        # The transposed_l looks like [ [], [] ] where the brackets contain an individual nt
        output_dictionary[sequencing_file] = list(zip(three_same_averages, three_rev_averages))
